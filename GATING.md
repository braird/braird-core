...
| `bindings/**`, `src/bin/uniffi-bindgen.rs`, `scripts/gen-bindings.sh`, `scripts/patch-kotlin-bindings.mjs` — the generated Swift/Kotlin surface + its round-trip tests (the public API devs consume), the one script that regenerates them, and the shim patcher that keeps a host `Throwable` from escaping the callback with the status unwritten (SUR-1014 — a crypto-review outcome, so weakening it must reach `crypto-reviewer`) | **GCE only** | Swift **and** Kotlin round-trip parity green + founder sign-off | Founder sign-off after `naming-reviewer` (the API *word*) **+** `crypto-reviewer` (the seam) pass |
