...
- `tests/` — `parity.rs` (the crypto parity harness, mirrors surfc's `crypto-parity.test.js`)
  and `schema_parity.rs` (the synced-schema drift reconciliation, SUR-723, plus the native-first
  schema lock and the no-third-category registry rule, SUR-1048).
...
- `vendored/schema/` — two fixtures with **opposite sources of truth**. `sync-schema.json` is
  DERIVED from `surfc/main` by `scripts/extract-sync-schema.mjs` and drift-guarded (SUR-723): surfc
  is authoritative, so CI re-derives it. `native-schema.json` + `native-manifest.json` are the
  native-first registry (SUR-1048): these tables have no PWA counterpart, so `store::native_schema()`
  is authoritative and the fixture is a hand-authored **lock** — change one, `tests/schema_parity.rs`
...
- `vendored/snapshot-parity/` — frozen PWA snapshot fixtures for schemas 1, 10, 11, 14, and 19
  plus a core-export fixture (SUR-911). Deterministic Rust tests consume them;
  `scripts/check-snapshot-parity.mjs` materializes clean oracle modules with `git show` from
  refreshed `origin/main` in the surfc checkout instead of trusting its worktree.
