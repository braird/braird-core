...
`release.yml` fires on a `v*` tag and attaches seven files to the GitHub Release for that tag:
...
| `great-ideas.json` | Vendored ordered `GREAT_IDEAS` canon from the tagged braird-core fixtures: 97 canonical leaf names validated against refreshed live `surfc/main` before publication | native canon consumers |
| `idea-tree.yaml` | Byte-vendored hierarchy metadata for the same canon; its leaf-name set is guarded against `great-ideas.json` | native canon consumers |
| `SHA256SUMS.txt` | `sha256sum` of the **six** payloads above (the four build/binding artifacts and two canon files) — computed from the exact published bytes | integrity verification |
...
**No floating `latest`. No tag-only pin.** Both the tag and the per-payload SHA-256 live in one
place in the consumer repo.

## Consumer pin — canon data (platform-neutral)

Treat `great-ideas.json` and `idea-tree.yaml` as one versioned unit. A consumer's lock/configuration
stores one exact release tag and the two SHA-256 values copied from that release's
`SHA256SUMS.txt`. Its bump tooling must:

1. Require both filenames and both pinned hashes, then download both files from that same exact
   release tag into a fresh temporary directory on the destination filesystem.
2. Hash both temporary files and fail closed if either expected hash is missing or either checksum
   differs. Delete the staged pair on failure; do not keep or fall back to either unverified file.
3. Only after **both** files verify, atomically install the pair (for example, rename a versioned
   directory or switch one manifest pointer). Never expose a new JSON with an old YAML, or vice
   versa.

Consumers must not fetch canon data from mutable `main`, a floating `latest`, or a tag trusted
without the two locally pinned hashes. `SHA256SUMS.txt` supplies the values during a reviewed bump;
it is not a substitute for recording those pins in the consumer.

The YAML contains only the four canonical top categories, eight canonical branches, and 97 leaf
assignments. The `bring` top and its `your_ideas` branch are host-generated by `ideaTree.js`, with
the current UI label **Yours**, and are deliberately absent from `idea-tree.yaml`. The YAML header's
historical wording **Ideas You Bring** describes that host-generated branch; it is not the UI label.
...
1. Cut the core release: bump `version` in `Cargo.toml`, add a `[X.Y.Z]` section to `CHANGELOG.md`,
   ...
   AAR + jar + xcframework zip + `BrairdCore.swift` + `great-ideas.json` + `idea-tree.yaml` +
   `SHA256SUMS.txt` in one create-only release (six hashed payloads, seven attached files).
2. In the app repo, open `chore(core): pin braird-core vX.Y.Z`: update the tag and **every checksum
   that app pins** together (including both canon hashes when it consumes canon), and let the app's
